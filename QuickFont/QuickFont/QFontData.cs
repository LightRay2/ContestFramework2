...
namespace QuickFont
{
    public class QFontData : IDisposable
    {
        ...
        public int maxGlyphHeightWithOffset;

        public int minGlyphOffset;
        /// <summary>
        ...
        {
            maxGlyphHeight = 0;
            maxGlyphHeightWithOffset = 0;
            minGlyphOffset = int.MaxValue;
            if (CharSetMapping.Count == 0)
                throw new Exception("шрифт не имеет ни одного символа в charset");
            foreach (var glyph in CharSetMapping)
            {
                maxGlyphHeight = Math.Max(glyph.Value.rect.Height, maxGlyphHeight);
                maxGlyphHeightWithOffset = Math.Max(glyph.Value.rect.Height + glyph.Value.yOffset, maxGlyphHeightWithOffset);
                minGlyphOffset = Math.Min(glyph.Value.yOffset, minGlyphOffset);
            }
