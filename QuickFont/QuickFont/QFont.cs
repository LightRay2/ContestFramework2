...
namespace QuickFont
{
    ...
    public class QFont : IDisposable
    {
        ...
        public QFont() { }
        public QFont(QFontData fontData) { this.fontData = fontData; }
        ...
        {
            get { return (float)Math.Ceiling(fontData.maxGlyphHeightWithOffset * Options.LineSpacing); }
        }
        ...
        public void Print(string text, float maxWidth, QFontAlignment alignment, Vector2 position, float maxHeight = float.MaxValue)
        {
            position = TransformPositionToViewport(position);
            position = LockToPixel(position);
            ...
            GL.PushMatrix();
            GL.Translate(position.X, position.Y, 0f);
            Print(text, maxWidth, alignment, maxHeight);
            GL.PopMatrix();
        }


        public void Print(string text, Vector2 position, float maxHeight = float.MaxValue)
        {
            position = TransformPositionToViewport(position);
            position = LockToPixel(position);
            ...
            GL.PushMatrix();
            GL.Translate(position.X, position.Y, 0f);
            Print(text, maxHeight);
            GL.PopMatrix();
        }

        public void Print(string text, QFontAlignment alignment, Vector2 position, float maxHeight = float.MaxValue)
        {
            position = TransformPositionToViewport(position);
            position = LockToPixel(position);
            ...
            GL.PushMatrix();
            GL.Translate(position.X, position.Y, 0f);
            Print(text, alignment, maxHeight);
            GL.PopMatrix();
        }


        public void Print(string text, float maxHeight = float.MaxValue)
        {
            Print(text, QFontAlignment.Left, maxHeight);
        }



        public void Print(string text, QFontAlignment alignment, float maxHeight = float.MaxValue)
        {
            PrintOrMeasure(text, alignment, false, maxHeight);
        }
        ...
        public SizeF Measure(string text, QFontAlignment alignment, float maxHeight = float.MaxValue)
        {
            return TransformMeasureFromViewport(PrintOrMeasure(text, alignment, true, maxHeight));
        }
        ...
        /// <returns></returns>
        public SizeF Measure(string text, float maxWidth, QFontAlignment alignment, float maxHeight = float.MaxValue)
        {
            var processedText = ProcessText(text, maxWidth, alignment);
            return Measure(processedText, maxHeight);
        }
        ...
        private SizeF PrintOrMeasure(string text, QFontAlignment alignment, bool measureOnly, float maxHeight)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                {
                    //added
                    if (Math.Abs(yOffset + 2 * LineSpacing) > maxHeight)
                        break; //дальше не рисуем
                    //---
        ...
        private void JustifyLine(TextNode node, float targetLength)
        {
            ...
            // we check how much additional length is added by adding an additional word to the line
        ...
        /// <param name="bounds"></param>
        /// <param name="alignment"></param>
        public void Print(string text, float maxWidth, QFontAlignment alignment, float maxHeight = float.MaxValue)
        {
            var processedText = ProcessText(text, maxWidth, alignment);
            Print(processedText, maxHeight);
        }
        ...
        private SizeF PrintOrMeasure(ProcessedText processedText, bool measureOnly, float maxHeight)
        {
            //find first line length here
            var popRequired = false;
            if (!measureOnly && !ProjectionStack.Begun && Options.TransformToViewport != null)
            {
                GL.PushMatrix();
            ...
            return new SizeF(maxMeasuredWidth, Math.Abs(yOffset + LineSpacing - yPos));
